    ...
    private static void createAndShowTransportSelectionGUI(String username) {
        ...
        nextButton.addActionListener(e -> {
            ...
            if ("Bus".equals(selectedTransport)) {
                createAndShowBusSelectionGUI(username);
            } else if ("Train".equals(selectedTransport)) {
                createAndShowTrainSelectionGUI(username);
            } else if ("Airplane".equals(selectedTransport)) {
                createAndShowAirplaneSelectionGUI(username);
            }
        });
        ...
        for (int i = 0; i < 30; i++) {
            ...
            seatButton.addActionListener(e -> {
                if (isSeatAlreadyBooked(username, transport + " - Seat " + seatNo)) {
                    ...
                    JOptionPane.showMessageDialog(frame, "Ticket booked for " + transport + " - Seat " + seatNo + " on " + dateField.getText());
                    String ticketInfo = transport + " - Seat " + seatNo + " on " + dateField.getText();
                    if (userTickets.containsKey(username)) {
                        userTickets.put(username, userTickets.get(username) + "," + ticketInfo);
                    } else {
                        userTickets.put(username, ticketInfo);
                    }
